...
After the imports are done, we can create the SMTP connection and login. The SMTP class takes two parameters here: the host and port to connect to; in this case, we are using Google's gmail service in particular. After creating this, a simple call to starttls() which tells our connection to encrypt the sent messages for safer communication. 
